...
JpegOptim.prototype.write = function (chunk) {
    ...
    if (!this.jpegOptimProcess && !this.bufferedChunks) {
        ...
        JpegOptim.getBinaryPath(function (err, jpegOptimBinaryPath) {
            ...
            this.jpegOptimProcess = childProcess.spawn(jpegOptimBinaryPath, ['--stdin', '--stdout'].concat(this.jpegOptimArgs));
            ...
            this.jpegOptimProcess.stderr.on('data', function (data) {
                if (!this.hasEnded) {
                    this._error(new Error('Saw jpegoptim output on stderr: ' + data.toString('ascii')));
                    this.hasEnded = true;
                }
            }.bind(this));
            ...
            this.jpegOptimProcess.stdout.on('data', function (chunk) {
